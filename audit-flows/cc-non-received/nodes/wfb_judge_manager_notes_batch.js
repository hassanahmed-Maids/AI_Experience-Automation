...
// --- call site: the manager+credit-notes fan-out (one call per red case) ------------------------------
...
// its batch on unchanged. Build Evidence Bundle reads $('Get Manager+Credit Notes') by name and is
// indifferent to this node existing.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures - LIVE. Five 5xx/429/timeouts in a row abort the batch. A trip here also stops the two SMS-log branches behind it under v1.
...
erpBreakerGuard({
  phase: 'Manager and credit notes (CC Non Received 2-Verify)',
  key: 'manager_credit_notes',
...
// $('Get Manager+Credit Notes') by name, so it neither sees nor cares about this node.
return $input.all();
