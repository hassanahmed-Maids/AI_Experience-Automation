...
// --- call site: the client-complaints fan-out (one call per red case) ------------------------
// This node exists ONLY to judge the batch and hand it on unchanged. Split Relevant Complaints
// is 5 KB of selection logic that reads $('Get Client Complaints') by name; pasting a generated
// 10 KB block on top of it would have made the interesting code the minority of the file and
// left the next reader scrolling past the part that matters. A node whose whole job is the
// breaker says what it is from the canvas.
//
// WHAT CAN AND CANNOT FIRE HERE, stated rather than implied:
//   consecutive_failures - LIVE. Five 5xx/429/timeouts in a row abort the batch, and because
//                          this is the FIRST fan-out of the batch, a trip here also stops the
//                          complaint-thread phase and all five note/SMS fan-outs behind it.
...
//                          so every batch is a separate PRODUCTION execution of THIS SAME
//                          workflow; run_id rides the baton unchanged, and
//                          $getWorkflowStaticData('global') persists between production
//                          executions on this instance. Batch 1 sets the baseline, batches
//                          2..n are judged against it. It is inert on batch 1, inert in manual
//                          test runs (static data is not written for those), and inert on any
//                          run small enough to fit in one batch of 50 - which, on this check,
//                          is most of them: the July 2026 residue was 4 cases. baseline_carried
//                          says which of those a given batch was.
//
// callsMade is CUMULATIVE across the batch, not this node's own count. Under executionOrder v1
// the six fan-outs run one after another, so by the time a later judge node runs, the elapsed
// clock from erp_t0 already contains every branch before it. Dividing that clock by one node's
// calls would read as "this node is N times slower than it is" and would swing with the number
// of complaint threads a batch happened to select. Dividing by the calls actually made turns it
// into a running mean per call over the whole batch - the same quantity in every batch, which
// is the only thing a baseline can honestly be compared against.
function _erpCallsSoFar() {
  const NODES = ['Get Client Complaints', 'Get Complaint Thread', 'Get Client Notes',
                 'Get Sales Todo Notes', 'Get Manager+Credit Notes',
                 'Get SMS Log (SMS)', 'Get SMS Log (WhatsApp)'];
  let n = 0;
  for (const nm of NODES) {
    // A node that has not run yet in this batch throws here; that is a zero, not an error.
    try { n += $(nm).all().length; } catch (e) { /* not executed yet */ }
  }
  return n;
}
const _responses = $input.all().map(function (i) { return i.json; });
erpBreakerGuard({
  phase: 'Client complaints list (CC Non Received 2-Verify)',
  key: 'complaints',
  responses: _responses,
  // 50, not 200. The policy's 200 exists to stop a small CANARY chunk from setting the baseline
  // for much larger chunks behind it. This flow has no canary: Assemble Baton fixes batch_size
  // at 50 and every batch is the same shape, so batch 1 is a like-for-like sample for batch 7.
  // The residual is the LAST batch of a run, which can be short and therefore amortises the
  // batch's fixed overhead over fewer calls; at 3x that is headroom rather than a trip, but it
  // is the direction a spurious latency trip would come from and it is not hidden here.
  minCallsForBaseline: 50
});
// =================== END ERP CIRCUIT BREAKER ===================

// Pass the batch on untouched. Split Relevant Complaints reads $('Get Client Complaints') by
// name and Attach Threads to Cases reads it too, so both are indifferent to this node existing;
// anything reading $input sees the same items in the same order. Returning the input items
// directly is n8n's passthrough and preserves paired-item lineage, which rebuilding them as
// {json: ...} would drop.
return $input.all();
