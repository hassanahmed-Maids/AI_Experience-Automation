...
// --- call site: the SMS-log fan-out, messageType SMS (one call per red case) ------------------------------
...
// its batch on unchanged. Build Evidence Bundle reads $('Get SMS Log (SMS)') by name and is
// indifferent to this node existing.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures - LIVE. Five 5xx/429/timeouts in a row abort the batch. A trip here also stops the WhatsApp branch behind it under v1.
...
erpBreakerGuard({
  phase: 'SMS log - SMS (CC Non Received 2-Verify)',
  key: 'sms_log_sms',
...
// $('Get SMS Log (SMS)') by name, so it neither sees nor cares about this node.
return $input.all();
