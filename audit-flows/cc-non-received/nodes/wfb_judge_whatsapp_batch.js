...
// --- call site: the SMS-log fan-out, messageType WHATSAPP (one call per red case) ------------------------------
...
// have judged five separate batches. One node per fan-out, each judging its own, each handing
// its batch on unchanged. Build Evidence Bundle reads $('Get SMS Log (WhatsApp)') by name and is
// indifferent to this node existing.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures - LIVE. Five 5xx/429/timeouts in a row abort the batch. This is the LAST ERP fan-out of the batch, so a trip here saves no further calls in this batch - what it saves is every batch after it, which is the whole remaining run.
...
//   latency              - LIVE FROM THE SECOND BATCH OF A RUN ONWARDS. Stage 2 self-calls per
//                          batch, so each batch is a separate production execution of THIS
//                          workflow, run_id rides the baton unchanged, and
//                          $getWorkflowStaticData('global') persists between production
//                          executions - batch 1 sets the baseline, batches 2..n are judged
//                          against it. Inert on batch 1, inert in manual test runs (static data
//                          is not written for those), and inert on any run small enough to fit
//                          in one batch of 50, which on this check is most of them.
//                          baseline_carried says which case a given batch was.
//
// callsMade is CUMULATIVE across the batch, not this node's own count. Under executionOrder v1
// the fan-outs run one after another, so the elapsed clock from erp_t0 already contains every
// branch ahead of this one; dividing it by 50 would report this node as several times slower
// than it is and would swing with the number of complaint threads the batch selected. Divided
// by the calls actually made it becomes a running mean per call over the batch - the same
// quantity every time, which is what a baseline needs.
...
erpBreakerGuard({
  phase: 'SMS log - WhatsApp (CC Non Received 2-Verify)',
  key: 'sms_log_whatsapp',
  responses: _responses,
  // 50, not 200 - see the note in Judge Complaints Batch. Every batch is the same 50 cases, so
  // there is no canary/full-chunk asymmetry for the policy's 200 to protect against. The short
  // LAST batch of a run is the residual risk and is named rather than hidden.
  minCallsForBaseline: 50
});
// =================== END ERP CIRCUIT BREAKER ===================

// Pass the batch on untouched into Join Evidence. Build Evidence Bundle reads
// $('Get SMS Log (WhatsApp)') by name, so it neither sees nor cares about this node.
return $input.all();
