...
// --- call site: the client-notes fan-out (one call per red case) ------------------------------
...
// its batch on unchanged. Build Evidence Bundle reads $('Get Client Notes') by name and is
// indifferent to this node existing.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures - LIVE. Five 5xx/429/timeouts in a row abort the batch. Under v1 this is the first of the five note/SMS branches, so a trip here also stops the four behind it - about four calls per remaining case.
...
erpBreakerGuard({
  phase: 'Client notes (CC Non Received 2-Verify)',
  key: 'client_notes',
...
// $('Get Client Notes') by name, so it neither sees nor cares about this node.
return $input.all();
