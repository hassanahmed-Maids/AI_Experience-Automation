...
// --- call site: the CPT fan-out (Get CPT Info) ------------------------------------------------
// The third and last per-entity read. It is judged separately from the two before it rather than
// once at the end, because the phases run in series: a trip here is 0 calls saved in this flow,
// but a trip at replacements or plan is one or two whole fan-outs not made. Judging only the
// last one would mean the breaker never once prevented a call.
//
// GET CPT INFO STAYS ON continueRegularOutput, DELIBERATELY — see the replacements call site.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures  - LIVE.
//   degraded_rate         - LIVE once the batch is >= 20 responses.
//   latency               - CANNOT FIRE. Key 'cpt' is measured once per run, so there is no
//                           earlier batch to baseline against; no erp_t0 is stamped either.
//                           `baseline_carried` logs false every run, by design and not by
//                           accident.
//
// WHAT A CLEAN VERDICT HERE DOES NOT MEAN. This is the last ERP read in 1-Score, but not the
// last in the run: 2-Verify makes up to eleven more calls per red case, in its own execution,
// under its own breakers. A green batch here says this flow stopped hitting a healthy ERP, not
// that the audit is safely over.
const _responses = $input.all().map(function (i) { return i.json; });
erpBreakerGuard({
  phase: 'CPT fan-out (CC Non Received 1-Score)',
  key: 'cpt',
...
// Pass the batch on untouched. Attach CPT reads its cases from a named node and pairs them
