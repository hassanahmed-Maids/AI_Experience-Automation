...
// --- call site: the replacements fan-out (Get Replacements) -----------------------------------
// This node exists ONLY to judge the batch and hand it on unchanged. The alternative was to put
// the breaker at the top of Attach Replacements, which is the node that pairs responses to cases
// POSITIONALLY and throws when the counts differ — a generated 10 KB block pasted above that
// would have buried the one assertion in this flow most worth reading. A node whose whole job is
// the breaker says what it is from the canvas.
//
// GET REPLACEMENTS STAYS ON continueRegularOutput, DELIBERATELY. Its failures have to arrive
// here AS ITEMS to be counted. Switching it to continueErrorOutput would route exactly the
// responses this breaker exists to see away from it and leave the breaker counting successes —
// a green verdict produced by hiding the evidence. It also keeps the one-item-per-input-item
// guarantee that Attach Replacements' positional pairing depends on.
//
...
//   degraded_rate         - LIVE once the batch is >= 20 responses. The enrichment cohort of a
//                           real month is in the thousands, so this is the threshold that will
//                           actually speak.
...
//                           the same run, and Get Replacements fans out over the whole cohort
//                           exactly once — there is no second batch to compare against, and
//                           Validate Inputs stamps no erp_t0, so elapsed is null and
//                           `baseline_carried` logs false. Said out loud because a latency check
//                           that silently never fires is the false-clearance shape this project
//                           keeps finding.
//
// AUTH IS NOT DEGRADATION. This endpoint is the one that returns INSUFFICIENT_PERMISSIONS on
// every call for an account without ClientReplacement. Those are counted and reported, never
// treated as a failing server — a breaker that counted them would trip on call five of every
// run ever fired, and the reaction to that is to raise thresholds until it detects nothing.
const _responses = $input.all().map(function (i) { return i.json; });
erpBreakerGuard({
  phase: 'Replacements fan-out (CC Non Received 1-Score)',
  key: 'replacements',
...
// Pass the batch on untouched. Attach Replacements reads its cases from a named node and pairs them
// POSITIONALLY against $input, and it throws when the two counts differ - so this node must
// return exactly the items it was given, in the same order, or it converts a healthy run into a
// pairing failure. Returning the input items directly is n8n's passthrough and preserves
