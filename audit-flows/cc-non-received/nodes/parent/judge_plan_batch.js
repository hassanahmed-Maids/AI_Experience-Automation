...
// --- call site: the contract-plan fan-out (Get Contract Plan) ---------------------------------
// A dedicated node for the same reason as the replacements one: Attach Plan is the positional
// pairing node and its assertion should stay the first thing a reader sees there.
//
// THIS IS THE MOST EXPENSIVE READ IN THE FLOW, and the reason the breaker earns its place here
// rather than only at the end. `clientmgmt/client/get-client-details` is the route measured at
// ~16 s and ~227 KB per 100-row page in ERP-LOAD-POLICY.md §1, and it is one of the two routes
// the 2026-08-19 clientmgmt incident took to nginx 503. If ERP is going to fall over under this
// run, it falls over here.
//
// GET CONTRACT PLAN STAYS ON continueRegularOutput, DELIBERATELY — see the replacements call
// site. Failures must reach this node as items or the breaker counts only the survivors.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures  - LIVE. Five 5xx/429/timeouts in a row abort the run, and this is the
//                           phase where that is most likely to be the truth about ERP.
//   degraded_rate         - LIVE once the batch is >= 20 responses.
//   latency               - CANNOT FIRE. One fan-out, once per run: there is no earlier batch of
//                           key 'plan' in this run to baseline against, and no erp_t0 is stamped,
//                           so ms_per_call is null and `baseline_carried` logs false. A green run
//                           here means the consecutive and rate rules passed; it says nothing at
//                           all about whether ERP got slower.
const _responses = $input.all().map(function (i) { return i.json; });
erpBreakerGuard({
  phase: 'Contract plan fan-out (CC Non Received 1-Score)',
  key: 'plan',
...
// Pass the batch on untouched. Attach Plan reads its cases from a named node and pairs them
