...
// --- call site: the complaint-thread fan-out (one call per selected complaint) ---------------
// A dedicated judge node, for the same reason as the others: Attach Threads to Cases is the
// fan-back-in anchor and carries the fetch-failure markers Rule 13 depends on. It reads
// $input.all() positionally against $('Split Relevant Complaints').all(), so this node returns
// its input untouched and that 1:1 pairing is unaffected.
//
// WHAT CAN AND CANNOT FIRE HERE:
//   consecutive_failures - LIVE. Under executionOrder v1 the complaints branch (topmost on the
//                          canvas) is walked to Join Evidence before the five note/SMS
//                          branches, so a trip here also stops those - about five calls per
//                          remaining case. That saving depends on the branch order; the trip
//                          itself does not.
//   degraded_rate        - CONDITIONAL, and more often inert here than anywhere else in the
//                          flow. This fan-out is one call per COMPLAINT WITH A THREAD, not per
//                          case, so its batch size is whatever Split Relevant Complaints chose:
//                          0 threads becomes the single keepalive sentinel call to
//                          .../historyOfComplaint/0, which ERP rejects and which arrives here
//                          as one degraded response. One response never reaches the 20-sample
//                          floor, so the sentinel cannot trip anything - which is the intended
//                          behaviour and worth knowing before someone reads a red line in the
//                          log and assumes the breaker is broken.
//   latency              - LIVE FROM THE SECOND BATCH OF A RUN ONWARDS. Same mechanism as the
//                          complaints fan-out: this stage self-calls per batch, so each batch
//                          is a separate production execution of the same workflow, run_id
//                          rides the baton, and $getWorkflowStaticData('global') carries the
//                          baseline between them. Inert on batch 1, in manual runs, and on any
//                          run that fits in a single batch of 50.
//
// callsMade is CUMULATIVE across the batch (complaints + threads by this point), not this
// node's own count: under v1 the elapsed clock from erp_t0 already contains the complaints
// fan-out, and the thread count itself swings with the month. Dividing the whole clock by the
// calls actually made is the only figure that means the same thing in every batch.
...
erpBreakerGuard({
  phase: 'Complaint threads (CC Non Received 2-Verify)',
  key: 'complaint_thread',
  responses: _responses,
  // 50, not 200 - see the note in Judge Complaints Batch. Every batch is 50 cases, so batch 1
  // is a like-for-like baseline; the short LAST batch of a run is the one shape that reads
  // slower per call, and it is named rather than hidden.
  minCallsForBaseline: 50
});
// =================== END ERP CIRCUIT BREAKER ===================

// Pass the batch on untouched. Attach Threads to Cases reads $input.all() and pairs it
// positionally against $('Split Relevant Complaints').all() - splits[i] to threads[i] - so the
// order and the count here are load-bearing. Returning the input items directly is the only
