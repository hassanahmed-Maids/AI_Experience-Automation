...
// ERP ISSUES TWO TOKENS PER LOGIN AND THEY ARE NOT INTERCHANGEABLE. The `authorization: Bearer`
// header and the `authTokenProduction` cookie carry DIFFERENT JWTs, minted seconds apart with
// different signatures (observed iat 11:12:19 vs 11:12:28 on the same device, 2026-08-20).
//
// This node used to derive the cookie from the bearer, which sends the same value for both and
// answers HTTP 500 / status 498 / "Access Token is missing or malformed <LOGOUT>" - a message
// indistinguishable from an expired session, so the operator logs in again, supplies the same
// mismatched pair, and fails identically. That cost a full diagnosis cycle on 2026-08-20.
//
// So the cookie token is taken from the payload. The old behaviour remains as a FALLBACK rather
// than an error, because a run whose two tokens genuinely match still works - but it is flagged,
// because the failure it causes is the least legible one in this system.
const cookieToken = String(auth.cookie_token || auth.token || "").trim().replace(/^Bearer\s+/, "");
const bearerBare = bearer.replace(/^Bearer\s+/, "");
const tokenBare = cookieToken || bearerBare;
const cookieTokenDerived = !cookieToken;

let tokenUser = null;
if (errors.length === 0) {
  try {
    const seg = bearerBare.split(".")[1];
...
if (cookieTokenDerived && errors.length === 0) {
  console.log(JSON.stringify({ stage: "ccprice_validate_warning", run_id: runId,
    note: "no erp_auth.cookie_token was supplied, so the authTokenProduction cookie is being " +
          "derived from the bearer. ERP issues TWO different tokens per login; if this run's " +
          "two do not match, every ERP call will answer 498 <LOGOUT>, which reads as an expired " +
          "session rather than a mismatched pair. Send erp_auth.cookie_token." }));
}
...
return [{ json: { ok: errors.length === 0, errors: errors, params: {
  ...
  erp_auth: { bearer: bearer, token_bare: tokenBare, cookie_token_derived: cookieTokenDerived, device_id: deviceId, acting_user: tokenUser }
} } }];
