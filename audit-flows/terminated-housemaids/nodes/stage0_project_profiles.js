// SLIM PROJECTION. The parent must never retain the raw profiles: each one is 154 keys, and
// a month is 80 maids. Only the fields the gates and the verifier actually read cross the
// boundary, so the parent holds kilobytes.
//
// GATE 30's ONE RETRY is n8n's retryOnFail (maxTries 2) on the node upstream, with neverError
// OFF so a 500 genuinely throws and therefore genuinely retries. An item that still failed
// after the retry arrives WITHOUT a statusCode and is scored reachable:false -> the parent
// records pending/erp_unreachable. NEVER a finding: an outage is not evidence of wrongdoing.
//
// responseFormat is 'autodetect', not 'json': ERP's error page is HTML, and under 'json' the
// node emits a parse-error item with NO statusCode at all - which makes a dead token
// indistinguishable from an unclassifiable anomaly.

const src = $('Expand Maids').all().map(function (i) { return i.json; });
if (src.length === 1 && src[0]._no_maids) return [{ json: { _no_maids: true } }];

const res = $input.all().map(function (i) { return i.json; });

// FAIL LOUD ON MISALIGNMENT. Responses are paired to maids BY INDEX, so a dropped or duplicated
// response would attribute one maid's termination record to another - and that produces a wrong
// finding about a named person. Never guess past this.
if (res.length !== src.length) {
  throw new Error('0-Fetch Profiles: ' + src.length + ' maids requested but ' + res.length +
    ' responses returned. Refusing to pair termination records to maids by index when the counts disagree.');
}

// ===================== ERP CIRCUIT BREAKER (ERP-LOAD-POLICY.md §5) =====================
// GENERATED - do not edit here. Canonical: audit-flows/tools/erp_breaker.js
// Re-generate with: python3 audit-flows/tools/build_breaker_embed.py --call-site plan --source-node "Expand Maids"
//
...
function erpBreakerGuard(opts) {
  const src = $('Expand Maids').first().json || {};
...
// --- call site: this chunk's profile reads --------------------------------------------------
// `res` is one item per maid in this chunk, in input order, straight from Get Housemaid Info.
// The parent calls this flow with mode:each over chunks of 25, so a trip here stops the
...
// the first chunk and applied to the rest of the same run.
erpBreakerGuard({
  phase: 'Project Profiles (Terminated HM 0-Fetch)',
  key: 'profiles',
  responses: res,
...
for (let i = 0; i < src.length; i++) {
  ...
  if (status !== 200) {
    out.push({ json: {
      maid_id: id, reachable: false,
      http_status: (status === undefined ? null : status),
      // Logged so a wrong pagecode is never mistaken for a missing permission: a wrong
      // pagecode returns 401 API_NOT_FOUND_FOR_PAGE, a real denial returns
      // INSUFFICIENT_PERMISSIONS, and both are otherwise just '401'.
      developermessage: hdrs.developermessage || hdrs.developerMessage || null,
      retried: true
    } });
    continue;
  }
  ...
  // resistanceNotes is the only free-text a person wrote that this profile exposes. It travels
  // for the verifier, capped. The complaint THREADS are unreachable (no complaint id is exposed
  // anywhere on this payload) - the parent labels affected verdicts complaint_threads_unread
  // rather than letting the agent answer 'explained' on evidence it never saw.
  const notes = lbl(b.resistanceNotes);

  out.push({ json: {
    maid_id: id, reachable: true, http_status: 200, retried: false,
    status: lbl(b.status),
    dateOfTermination: lbl(b.dateOfTermination),
    modeOfTermination: lbl(b.modeOfTermination),
    reasonOfTermination: lbl(b.reasonOfTermination),
    resistance_notes: notes ? notes.slice(0, 600) : '',
    has_written_record: !!notes,
    complaint_counts: {
      maid_against_all_clients: b.maidAgainstAllClientsComplaintsNumber === undefined ? null : b.maidAgainstAllClientsComplaintsNumber,
      client_against_maid: b.currentClientAgainstMaidComplaintsNumber === undefined ? null : b.currentClientAgainstMaidComplaintsNumber,
      maid_against_current_client: b.maidAgainstCurrentClientComplaintsNumber === undefined ? null : b.maidAgainstCurrentClientComplaintsNumber
    }
  } });
}
return out;
