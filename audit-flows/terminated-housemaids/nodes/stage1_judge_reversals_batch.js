...
// the breaker at the top of Score Cases, which is 22 KB of gate logic - a generated
// 10 KB block pasted into it would have made the interesting code the minority of the file, and
...
//                           reversal REFERENCE, not per transaction - a quiet month can be well
//                           under 20, in which case only the consecutive rule is watching. That
//                           is the honest position, not a gap to paper over: with 5 consecutive
//                           failures out of a batch of 12 the run still aborts.
...
//                           the same run, and Get All-Time Reversals fans out over the whole
//                           population exactly once - there is no second batch to compare to.
...
erpBreakerGuard({
  phase: 'All-time reversals (Terminated HM 1-Score)',
  key: 'reversals',
...
// Pass the batch on untouched. Score Cases reads $('Get All-Time Reversals') by name, so it is
// indifferent to this node existing; anything reading $input sees the same items in the same
// order. Returning the input items directly is n8n's passthrough and preserves paired-item
// lineage, which rebuilding them as {json: ...} would drop.
return $input.all();
