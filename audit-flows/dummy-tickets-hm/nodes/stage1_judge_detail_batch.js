...
// the breaker at the top of Resolve Applicants, which is 5.3 KB of identity logic - a generated
...
//                           (a month of dummy-ticket transactions).
...
//                           the same run, and Get Transaction Detail fans out over the whole
...
erpBreakerGuard({
  phase: 'Transaction detail (Dummy Tickets HM 1-Score)',
  key: 'detail',
...
// Pass the batch on untouched. Resolve Applicants reads $('Get Transaction Detail') by name, so
// it is indifferent to this node existing; anything reading $input sees the same items in the
// same order. Returning the input items directly is n8n's passthrough and preserves paired-item
