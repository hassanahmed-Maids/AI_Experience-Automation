...
// 2026-08-24. Execution 100409 read 0 of 399 applicants: `Get Hustler Tickets` returned 401 on
// every call, the fan-out ran all 399 anyway with maxTries 2 (~800 requests), and the run then
// reported `overall: pass` with `applicants_unreachable: 399`. Three runs that day, ~2,400
// requests to production ERP, not one of which could have succeeded.
//
// With chunks of 25, the wall now trips on the FIRST chunk: 25 calls instead of ~800. That is
// the whole saving available - the HTTP node returns only when its last request is done, so
// nothing of ours runs during a chunk and call 6 of 25 cannot be stopped.
//
// pagecode is DECLARED here because it cannot be observed: n8n puts neither the request headers
// nor the response headers on an error item, so the breaker has no way to know what it asked
// for. Naming it turns "401" into "this identity lacks RECRUITMENT__HustlersWorkflow", which is
// the sentence the operator has to take to whoever grants permissions.
erpBreakerGuard({
