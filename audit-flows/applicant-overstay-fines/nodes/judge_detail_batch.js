...
// Re-generate with: python3 audit-flows/tools/build_breaker_embed.py --call-site plan --source-node "Build Run Context"
//
...
function erpBreakerGuard(opts) {
  const src = $('Build Run Context').first().json || {};
...
// A node whose ONLY job is to judge this batch and hand it on unchanged. The alternative was to
// put the breaker at the top of `Attach Identity`, which is a 1.4 KB Run-Once-for-EACH-Item node
// - a 10 KB generated block cannot live there at all (it would re-evaluate per item and re-read
// static data every time), and in the larger scorers downstream it would make the interesting
// code the minority of the file. A node whose whole job is the breaker says what it is from the
// canvas.
//
// WHY `Get Transaction Detail` IS ON continueRegularOutput AND NOT continueErrorOutput. Its
// failures have to arrive HERE as items to be counted. Routed out of the error output they would
// go to the lease rail instead, and this breaker would sit downstream of a node that only ever
// hands it successes - a detector counting the one population that cannot contain the thing it
// detects. The breaker wins; the lease rail still covers every Code node on this path.
//
// WHAT CAN AND CANNOT FIRE HERE, stated rather than implied:
//   consecutive_failures  - LIVE. Five 5xx/429/connection timeouts in a row abort the run, at any
//                           batch size. This is the threshold that actually protects ERP here.
//   degraded_rate         - LIVE ONLY WHEN THE WINDOW IS BIG ENOUGH. It needs >= 20 responses and
//                           the batch is one call per OVER-BASE transaction, not per cohort row:
//                           measured 2026-08-12, nine months of cohort (963 rows) yielded 65
//                           over-base transactions, and May and June yielded ZERO. A one-month
//                           run is therefore usually under 20 and this threshold is inert; the
//                           default six-month window clears it. Said out loud because "the rate
//                           rule is on" and "the rate rule can fire today" are different claims.
//   latency               - CANNOT FIRE, and that is not a defect. The rule compares this batch
//                           against a baseline taken from an EARLIER batch of the SAME key in the
//                           SAME run, and this fan-out runs exactly ONCE per run - there is no
//                           second batch to compare against. `erp_t0` is therefore deliberately
//                           not stamped, `ms_per_call` logs null and `baseline_carried` logs
//                           false. A latency check that silently never fires is the
//                           false-clearance shape this project keeps finding, so it is named
//                           rather than left to be inferred from a green run.
const _responses = $input.all().map(function (i) { return i.json; });
erpBreakerGuard({
  phase: 'Transaction detail (CC Overstay Fines)',
  key: 'detail',
...
// Pass the batch on untouched. `Attach Identity` reads `$json` and `$('Split Transactions').item`,
// so it is indifferent to this node existing - but only while the items arrive in the same order
// with their pairing intact. Returning the input items directly is n8n's passthrough and
// preserves that lineage; rebuilding them as `{ json: ... }` would drop it.
return $input.all();
