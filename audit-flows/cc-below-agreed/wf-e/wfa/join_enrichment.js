...
let planFailures = 0, replFailures = 0, replDenied = 0, replDead = 0;
const out = cases.map(function (c) {
  ...
  if (d.replacements_meta && d.replacements_meta.token_dead === true) replDead++;
...
// WF-E throws on a dead token, so this should be unreachable - it is here because "should be
// unreachable" is how a chunk from an older published version of WF-E gets through. A single
// dead-token case means every read after it was empty, and an empty maid history scores as
// "no change": cases would clear on evidence nobody fetched.
if (replDead > 0) {
  throw new Error('Join Enrichment: ' + replDead + ' of ' + cases.length + ' cases carry ' +
    'token_dead from WF-E. The bearer died mid-enrichment, so those cases were scored against ' +
    'empty reads. Re-issue the token and re-run rather than publishing a run with a hole in it.');
}
...
console.log(JSON.stringify({ stage: 'join_enrichment', cases: out.length,
  ...
  replacement_permission_denied: replDenied, replacement_token_dead: replDead,
