...
-- A1. 🔴 O-D — AIRFARE PAID ABOVE ITS NATIONALITY TIER. The amount is a flat per-nationality
--   constant, so anything above the tier is money over entitlement. Splits ABOVE from BELOW:
--   the audit's axis is overpayment, and lumping them as "off-tier" hides which is which.
--   A7b confirmed the live tiers: Filipina 2,000; Ethiopian / Ugandan / Kenyan 1,500.
--   ⚠️ The parameter store is not in the warehouse, so tiers are recovered from the data's own
--   modal amount. A whole nationality paid the wrong flat rate would look correct here — this
--   test finds outliers against practice, not against config.
WITH air AS (
    ...
    SELECT nationality, MODE(AMOUNT) AS tier, COUNT(*) AS notes
    FROM air GROUP BY 1
)
SELECT t.nationality,
       t.tier                                                        AS nationality_tier,
       t.notes                                                       AS notes,
       COUNT_IF(a.AMOUNT > t.tier)                                   AS paid_ABOVE_tier,
       ROUND(SUM(IFF(a.AMOUNT > t.tier, a.AMOUNT - t.tier, 0)))      AS aed_OVER_ENTITLEMENT,
       ROUND(MAX(IFF(a.AMOUNT > t.tier, a.AMOUNT - t.tier, 0)))      AS largest_excess,
       COUNT_IF(a.AMOUNT < t.tier)                                   AS paid_below_tier,
       COUNT(DISTINCT a.AMOUNT)                                      AS distinct_amounts
FROM air a JOIN tier t ON t.nationality = a.nationality
GROUP BY 1, 2, 3
ORDER BY aed_OVER_ENTITLEMENT DESC, t.notes DESC;
