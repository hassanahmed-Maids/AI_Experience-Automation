...
| **O-C2** | 🔴 **RE-TEST RAN AND FOUND A SCOPING DEFECT, NOT A CONFIRMATION.** The corrected query returned **40 maids, all reading `NO referral record at all`** — `authorised_aed = 0`, so `paid > authorised` fired automatically, summing to ~AED 58,000 against C2's 11,500. That is one mis-scoped rule, not 40 findings, and the `entitlement_basis` guard column is what caught it. **What it exposed is structural**: `REASON = 'Bonus'` carries at least **seven distinct purposes** (see §1 coverage), of which `HOUSEMAID_REFERRALS` prices only two. **C2 must be scoped by an INNER join to maids who hold a referral entitlement**, not by a `>` comparison that treats absence as zero. Re-run scoped, and check `paid_over_authorised_ratio` clusters at **2.00** — O12b's 16 maids were each paid *exactly double*. If it does not cluster, the 11,500 is re-derived, not confirmed | Audit | **No — before publication** |
