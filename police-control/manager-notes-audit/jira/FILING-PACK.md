...
| ✅ **N17 — RESOLVED 2026-09-09** | **`HOUSEMAID_TYPE_LOGS`** is the contract-type timeline: `HOUSEMAID_ID, FROM_TYPE, TO_TYPE, CHANGE_DATE, PREV_CHANGE_DATE, NEXT_CHANGE_DATE` — closed intervals, so the read is plain containment. It also carries `CC Live In` / `CC Live Out` / `MV` directly, which **resolves N19 in the same column**. No business ask, no ingestion. *(The old entry pointed at `HOUSEMAIDS_INFO_REVISION`, whose type columns are all empty.)* | the ELIG archetype — 9 payment types — **unblocked** |
