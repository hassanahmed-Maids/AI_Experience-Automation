...
| 🔴 **Bonus over the referral entitlement** | **11,500** *(as of 2026-09-15)* | not deserved | **16 maids, AED 11,500 — RE-DERIVED INDEPENDENTLY 2026-09-15 and confirmed to the dirham.** Seven over by 1,000, nine by 500. ⚠️ **The old basis line said "every one paid exactly double her entitlement" and that is WRONG** — a pattern read off the modal case and generalised. The ratios are **12 of 16 at 2.0x, three at 3.0x** (maids 35276, 80030, 84374 — each paid 1,500 against 500 authorised) **and one at 1.5x** (maid 112692, 1,500 against 1,000). ✅ **Transfer exposure is ZERO** — no note in this check sits on a payslip that failed to transfer, so O-TRANSFER cannot move it. ✅ **Scoped by an INNER join to maids who hold a referral entitlement.** A `paid > authorised` comparison that treats absence as zero returns **40 maids / ~AED 58,000**, because `REASON = 'Bonus'` carries at least seven purposes and `HOUSEMAID_REFERRALS` prices only two of them. ⚠️ Was 15 maids / 10,500 on 2026-09-08: the window is ROLLING, so this row drifts with the date. **8 of the 16 maids, carrying AED 6,000 of the 11,500, also appear in the control row "paid before the bonus was requested"** (confirmed 2026-09-15; the note-level figure is 9 notes / AED 6,500) — different tables, so no double-count, but **half this row's money tells a second story about the same people** |
...
| 🔴 **Anti-attrition paid to a maid in a NO-SHOW or terminated state** | **1,613** *(was 13,257 until 2026-09-15)* | not deserved | **110 notes (S4).** 68 `NO_SHOW_WENT_OUT_DID_NOT_RETURN` (8,147) · 21 `NO_SHOW_LEFT_CLIENT_HOME` (3,672) · 13 `NO_SHOW_FOR_TERMINATION` (748) · 6 `NO_SHOW` (419) · 2 `EMPLOYEMENT_TERMINATED` (271). 🔴 **FRAMING CORRECTED 2026-09-15 (B3e, conv 46467).** I called this a select-once flaw. It is not — for 108 of the 110 notes there is no gap to fall through. `Housemaid.rejectedStatuses` (`Housemaid.java:120-127`) is exactly seven values — `EMPLOYEMENT_TERMINATED`, `WAITING_TO_FILE_ABSCONDING`, `VISA_UNSUCCESSFUL`, `REJECTED`, `UNREACHABLE`, `UNREACHABLE_AFTER_EXIT`, `PASSED_EXIT` — and **no `NO_SHOW*` variant is among them**. The job is DESIGNED to pay absconded maids; they are never filtered, at selection or anywhere. So the row splits: **108 notes / AED 12,986 in the four `NO_SHOW*` states are deliberate config, not a defect** (a policy question, if anyone wants to ask it), and **2 notes / AED 271 in `EMPLOYEMENT_TERMINATED` are the only genuine select-once cases** — that status IS in the list, so selection should have excluded maids 53238 and 121567, both terminated on the payment day itself. Even those two sit in the pay-period-uncertain bucket. ✅ **CONFIRMED A SECOND TIME 2026-09-15, on the PAYMENT date rather than the note date** (`queries/absconded-payment-date.sql`). Every earlier cut resolved status as of the NOTE date; payroll pays 1-3 days later, so the column I had been calling `status_when_paid` was really *status when the note was written*. Re-measured on the day the money actually moved: **84 of 110 notes changed status in between, almost all toward RECOVERY** — `NO_SHOW_*` → `WITH_CLIENT` / `AVAILABLE` / `ON_VACATION`. **63 notes / AED 7,844 went to maids who were back at work when payroll ran** and are not a finding in any form; **13 notes / AED 1,097 sit on payslips with `IS_TRANSFERRED = 'NO'` — that money never left the company**; 5 more are an invalid fallback and 7 have no establishable payment date. The hard core is **8 notes / 5 maids / AED 1,970.96 gross**, durably absent *and* transferred. Net of what she had earned that is **AED 1,612.90 — the ledger row to the dirham**, reached by a completely independent method with no entitlement proxy anywhere. Two methods, one number. A `NO_SHOW` flag on the 31st that reads `WITH_CLIENT` by the 3rd is a transient month-end state, not abscondment. 🔴 **RE-SCOPED 2026-09-15 — B3b limb (i) is answered: the ERP prorates and that logic is correct, not to be questioned. The row is AED 1,613, not 13,257.** Prorating each note against the days she was actually eligible (`queries/anti-attrition-abscondment-cases.sql`) splits the 110 notes into **AED 10,210 she had already worked for** and **AED 1,647 for days after she left** — and of that 1,647, **AED 1,250 is a pay-period bug** (14 notes dated the 1st, where the month-truncation forces days-eligible to zero although 11 show `WITH_CLIENT` the cycle before), **AED 280 is the entitlement proxy failing** (42 implied days in a 30-day month) and **~AED 183 is last-day rounding** (65 of 105 notes have days-gone = 0). **AED 1,612.90 survives, across three maids** — 73378 (600), 110872 (852), 103699 (161). A further **AED 2,566 / 42 notes is not estimable at all** (never paid a whole month, so no proxy exists). What remains of the row is **two defects, not a proration dispute**: a maid **enrolled while already absent** (73378 — enrolled 13 days after she absconded; ✅ **CONFIRMED 2026-09-15**, see B3d) and **no eligibility re-check between cycles** (full unprorated months to maids gone 37-98 days) |
...
| **Airfare to MV maids** | **4,500** | off-rule | **3 notes (AF2).** RED = MV across the **whole 24-month entitlement window**, not merely at the note date — a far stricter test than the point read that produced S5's AED 137,500 candidate, of which **97% was confound** (airfare notes are dated `payrollDueDate`, so a maid CC at renewal and MV by the payroll cycle reads as MV). ✅ **Not exposed to the B3g month-boundary confound** — a maid MV for 24 months cannot have converted at a recent month end. ✅ **RECONCILED 2026-09-15 — the 6,000 is NOT a competing measure and has been retired.** Both tests were run on the same data: **RED(24m) ⊆ RED(5m), 3 of 19**. The 24-month test still returns exactly **3 notes / AED 4,500** (2026-04-01 to 04-29); the 5-month test returns **19 notes / AED 34,500**, and the old "3 notes / 6,000" is the three inside it dated **2026-09-01** (maids 34326, 32449, 33960 at AED 2,000 each). **Disjoint sets, different questions.** The 5-month window is *Guard 1's duplicate window*, not the entitlement window — the gate `!housemaid.isMaidVisa()` is evaluated when the entitlement is created at renewal, while the note is dated `payrollDueDate` months later. All 16 five-month-only notes carry **1-2 CC intervals inside the prior 24 months** (maid 34326 switched type 2026-03-31, note dated 2026-09-01), so the 5-month test **reintroduces the exact confound AF2 was built to defeat** — the one that collapsed 137,500 → 4,500. Row 5 stands at **4,500**. The 16 notes / AED 30,000 in the gap are the conservative floor's known cost and remain candidates, not findings. |
...
| 5 | Airfare to MV maids | 4,500 | off-rule | AF2 · 3 notes · MV for the **whole 24-month window**, so **not** exposed to the B3g month-boundary confound. ✅ **Reconciled 2026-09-15** — the 6,000 was a 5-month test on a disjoint note set and is retired; see the detail row |
...
| 🔴 **Anti-attrition enrolment can be deleted without privilege or trace** *(new 2026-09-15)* | *no amount* |
| **Subtotal** | **16,626** |

🔴 **The delete-guard gap (conv 46470).** On `/maidNote`, `createEntity` (:116) and `updateEntity` (:147) both enforce the incentive type check, `validateIncentiveExperimentPositionAccess` and `validateIncentiveAmount`. **`deleteEntity` (:164-166) enforces none of them, and carries no `@PreAuthorize`** — it calls `super.deleteEntity(log)` unguarded. So enrolling a maid onto a money-bearing programme is position-restricted, changing her amount is position-restricted, and **erasing the enrolment entirely is not**. The entity has no `@Audited`, no soft-delete flag and no history table, so the deletion leaves nothing behind. **18 maids / 42 notes / AED 9,019 are the population where this demonstrably happened** — they were paid, so an enrolment existed; their earliest surviving row postdates the payment, so the original is gone. Carries no amount: the money was probably owed. ⚠️ Residual: whether `magnamedia-core` applies framework-level Envers auditing to this entity is not readable from the housemaid-management repo — if it does, the deletions are recoverable and this is downgraded.
...
2. ✅ **Airfare-to-MV RECONCILED 2026-09-15 and closed at AED 4,500.** The rival 6,000 was a
   **5-month** lookback; AF2 uses **24 months**. Run together: RED(24m) ⊆ RED(5m), 3 of 19 — the
   24m test still gives 3 / 4,500, the 5m test gives 19 / 34,500, and the old "3 / 6,000" is the
   three inside it dated 2026-09-01. Disjoint sets, different questions. The 5m window is Guard 1's
   duplicate window, not the entitlement window, and all 16 extras were CC inside the prior 24
   months — the 5m test reintroduces the confound AF2 exists to defeat. No longer an open item.
