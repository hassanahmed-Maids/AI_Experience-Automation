...
| **Status** | Draft — blocked on the warehouse grant. 12 requestor decisions open (§7) |
| **Last amended** | **2026-09-07** — business rules from George Abboud via Hassan Ahmed (airfare CC tenure, referral scheme, accommodation relocation), plus Snowflake access findings. See "What changed 2026-09-07" |
...
1. 🔴 **Airfare is `cc_months >= 22`, CC only — not the code's `% 24 == 22`.** The code rule is true
   ...
   airfare payments. A2 and A3 are rewritten; the divergence itself becomes **an outstanding ask**.
...
> verified **nowhere**. Every such claim is marked `NEEDS COMPUTE` and is.
...
- **B5 — enrolment amount is allowed.** `incentiveAmount ∈ MAID_INCENTIVE_CONFIGS_PARAM.amount_values`.
  🔴 Read the parameter live, never the code default — production has demonstrably extended it. **(CEIL)**
...
column that the view does not expose**. Do not report group B as blocked wholesale: four of
...
- 🔴 **AED 1,200 is unexplained and must not be judged.** It is profiled in **both** tables; the
  payment side also shows 250, 1,500 and 2,000, and the referral side shows 0. Until an outstanding ask answers,
