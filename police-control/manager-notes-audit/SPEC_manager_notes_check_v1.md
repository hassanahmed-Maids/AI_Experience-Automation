...
| **B3b** | ✅ **ANSWERED 2026-09-15 (limb i) — the ERP's proration is CORRECT and is not to be questioned.** A maid who absconds mid-month **keeps the prorated part she worked**; only the part paid for days after she left is a finding. **Row 1 collapses from AED 13,257 to AED 1,613**, three maids. The remaining 110-note population is not a finding — AED 10,210 of it was earned, and the residue was my own pay-period bug (1,250), the entitlement proxy failing (280) and last-day rounding (183). See `queries/anti-attrition-abscondment-cases.sql` | Requestor → management | **Closed** |
| **B3d** | ✅ **ANSWERED 2026-09-15 — `CREATION_DATE` is trustworthy; the finding stands.** Asked whether the low-code layer delete+creates an incentive action-log row on edit (which would make the date an edit, not an enrolment). conv 46467 could not read it — `magnamedia-core` is a compiled dependency with no source in the workspace — but the **data settles it**: 43 enrolment rows carry `CREATION_DATE <> ACTION_DATE`, and since `createEntity` stamps `actionDate` to today, a row where they differ can only be one **edited in place with `creationDate` preserved**; under delete+create both would be stamped fresh and equal. Maids also hold up to **7** enrolment rows, where delete+create would leave one. **Maid 73378 was genuinely enrolled 13 days after she absconded** | — | **Closed** |
| **B3e** | ✅ **ANSWERED 2026-09-15 (conv 46467) — and it corrects the framing of the whole finding.** `Housemaid.rejectedStatuses` (`Housemaid.java:120-127`) holds seven values and **no `NO_SHOW*` variant**. So paying an absconded maid is **deliberate config, not a select-once defect**: 108 notes / AED 12,986 are the system working as declared. Only the **2 notes / AED 271** in `EMPLOYEMENT_TERMINATED` — a status that IS in the list — are genuine select-once cases. ⛔ **THE POLICY QUESTION IS CLOSED — the requestor closed it 2026-09-15 and it will not go to management.** It was raised as *"should a `NO_SHOW` maid be excluded from a retention incentive at all?"* and the payment-date re-measure had already undercut it: **63 of the 108 notes went to maids who were back at work on the day payroll ran**, so "the system deliberately pays absconded maids" overstates what happens. `NO_SHOW` at month-end is overwhelmingly a transient state that resolves within days. **No further B3 work on the `NO_SHOW*` population** | Requestor | **Closed** |
| **B3f** | ⚪ **ROW 3 RETRACTED TO CANDIDATE 2026-09-15 — AED 9,019 is unfalsifiable from the warehouse.** The job selects on `EXISTS` against the enrolment row, so a maid cannot be paid without one; a note predating her earliest surviving row means the original row is **gone**. `evidence-antiattrition-actionlog-conv46015.md` §1 records `deleteEntity` as a base-controller delete with **no soft-delete flag and no history snapshot**, so "never enrolled when paid" and "enrolled, unenrolled, re-enrolled" are **indistinguishable in data**. Three discriminators failed first — expense head (same for all 9,143; §3 had already said the manual route lands under the same addition reason, so this was predictable), requester pool (7 of the same 26), approval (**100% on both sides** — the note only exists once the expense todo is `confirmed`, so approval is the creation mechanism, not human authorisation). Money-lost total falls 44,560 → **35,541**. Restore only if the code confirms enrolment rows cannot be deleted without trace — conv 46470 has now CONFIRMED it will not be restored: the only removal path is an unguarded hard delete with no Envers, no soft-delete flag and no history write, and no path can back-date creationDate. **The 42 notes convert into a CONTROL finding** — `deleteEntity` (:164-166) enforces none of the incentive guards that `createEntity` (:116) and `updateEntity` (:147) enforce, so enrolment history for a money-bearing programme can be erased without privilege or trace | Ask-the-code | **Closed** |
| **B3g** | 🔴 **NEW — ledger row 4 (AED 5,526) may collapse to AED 426.** Re-measuring B3c surfaced that **18 of the 20 notes went to maids who switched to MV on the LAST DAY of a month** (AED 5,100), each paid a **full whole-entitlement amount** (200/300/350/400) within 0-18 days of switching — i.e. the full incentive for a month spent entirely as CC, paid in arrears. That is the shape B3b limb (i) already settled (*she keeps what she earned*), and the variable 0-18 day gap is what two async hops between selection and note creation produce — **B3a's selection-lag ruling with a longer lag than the arbitrary 2-day line**. Only maids **104507** (126) and **38994** (300) switched mid-month. ⚠️ **NOT retracted: the base rate is untested.** If maids.cc always converts at month-end, 18-of-20 is the base rate and carries no information — the same trap as the expense-head and approval tests on row 3. ✅ **GATE PASSED AND ROW 4 RETRACTED 2026-09-15.** Base rate of month-end MV switching: **10.6%** (1,349 of 12,741). This row: **90%**. An **8.5x enrichment**, P(>=18 of 20) ≈ **4.4e-16** — the concentration is real and the innocent explanation is positively supported, not merely unexcluded. **Row 4 drops 5,526 → 426**, two notes (104507 at 126, 38994 at 300). Ledger total 35,541 → **30,441** | Audit | **Closed** |
| **B3c** | ✅ **RE-MEASURED 2026-09-15 — AED 2,476 → AED 126, one note, one maid.** The old cut was two-way at 2 days, so everything past it was labelled "MV well before payment"; a three-way cut shows almost all of those maids were **3-18 days** into MV, not months. Only maid **104507** is genuinely long-gone (MV since 2025-08-26, paid 2025-11-18, **84 days**, AED 126) — and she is also one of only two in the row who did not switch on a month boundary. ⚠️ **This re-measure raises a larger question about ledger row 4 itself — see B3g** | Requestor | **Closed** |
