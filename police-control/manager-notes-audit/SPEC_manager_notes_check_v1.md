...
| **B3f** | 🔴 **NEW 2026-09-15 — ledger row 3 (AED 9,019, 42 notes) may be measuring the wrong thing.** It is named "paid before any enrolment existed", which assumes every anti-attrition note comes from the enrolment-driven job. `evidence-antiattrition-actionlog-conv46015.md` §3 records that the **Abu Dhabi incentive job** and any **manual `AAI - 01` expense request** produce the same addition reason with **no enrolment row required at all**. The B3d test found 8 maids / 9 notes with no enrolment row whatsoever, and before-enrolment cases concentrate 8x among long-tenured maids — the population those producers would hit. Re-test row 3 by expense head and requester before it is reported | Audit | **Yes for row 3** |
