...
| **B3d** | 🔴 **BLOCKED ON CODE, NOT ON A HUMAN — may a maid be ENROLLED onto the incentive while already absent?** Maid 73378 absconded 2025-09-24 and her `Maid_Incentive_Experiment` action-log row was created 2025-10-07, *thirteen days after*; she was then paid AED 200 a month for three months. ⚠️ **Do not put this to anyone yet.** The reading rests on `CREATION_DATE`, which is right *if* an edit updates the row in place. `evidence-antiattrition-actionlog-conv46015.md` §1 states that whether the UI/low-code layer instead issues **delete + create** is **unverified** — if it does, 2025-10-07 is an amount edit, not an enrolment, and this finding is an artefact. Resolve by asking the code first | Ask-the-code → then management | **Yes** |
| **B3e** | 🔴 **BLOCKED ON CODE — is NO_SHOW inside `Housemaid.rejectedStatuses`?** The selection query already filters `h.status NOT IN :inactiveStatuses` (= `rejectedStatuses`). This single fact decides **who owns the question**: if NO_SHOW is NOT in the list, the system deliberately pays absconded maids and that is a policy question for management; if it IS in the list, the job should have skipped maid 73378 three cycles running and did not — a **defect for the devs**, which nobody needs to rule on. Do not ask a human until this is read | Ask-the-code | **Yes** |
