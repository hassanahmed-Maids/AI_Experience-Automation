...
description: >
  This skill should be used to establish whether the raw data an auditing dashboard needs
  already exists in the maids.cc Snowflake warehouse, and to write the data ingestion request
  for whatever is missing. Triggers on "does this data exist in Snowflake", "check Snowflake
  for", "which Snowflake table has", "find the table for", "is this already in the
  warehouse", "what do we need ingested", "write the ingestion request", or whenever a spec
  needs a verified table or column name. Also use when a metric may already have an approved
  KPI definition that must be reused rather than rebuilt. Covers discovery queries, grain and
  freshness checks, type traps, the approved-KPI rule, and the ingestion request format.
metadata:
  version: "0.1.0"
...
Run these in order. Read `references/discovery-queries.md` for the full query set.
...
`references/ingestion-request.md`. Where the source is the ERP, get the native table and
column names from the **`ask-the-code`** skill first — an ingestion request naming a real
