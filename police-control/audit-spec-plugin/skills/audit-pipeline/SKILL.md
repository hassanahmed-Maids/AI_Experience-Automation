...
|---|---|---|
| "we need to control X", "build a dashboard that audits X" | **specify** | `audit-spec-builder` |
| "does Snowflake have X?", "what tables hold X?" | **discover** | `snowflake-discovery` |
| "how does the ERP decide X?", "which job sends X?" | **discover** | `ask-the-code` |
| "what should the report look like?" | **specify** | `audit-report-ui` |
| "run it", "give me the query", "is this a finding?", "should we report this?" | **execute** | `audit-execution` |
| "file it", "hand it to DNA", "write the ticket" | **hand over** | `audit-spec-builder` → `references/dna-handoff.md` |
...
| Skill | Read it for |
|---|---|
| `snowflake-discovery` | what exists in the warehouse; metadata-only technique when no grant exists; the ingestion ask |
| `ask-the-code` | interrogating the ERP; ⚠️ a module-scoped question that finds nothing is evidence about the **module**, never the ERP |
| `audit-spec-builder` | the spec pipeline, the eighteen spec defects, the DNA handoff |
| `audit-report-ui` | the report surface and its patterns |
| `audit-execution` | the pre-flight, fourteen worked failures with real numbers, six SQL patterns with their guards, the verdict model |
