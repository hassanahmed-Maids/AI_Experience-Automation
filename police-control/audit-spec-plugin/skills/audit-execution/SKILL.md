...
description: >
  Use this skill whenever a Police & Control audit moves from being SPECIFIED to being RUN —
  when anyone is about to write or run SQL against Snowflake to find money that should not have
  moved, adjudicate what a query returned, or decide whether a number is publishable. Triggers on
  "run the audit", "check these payments", "did anyone get paid twice", "find the overpayments",
  "audit manager notes", "is this a real finding", "give me the query for X", "what does this
  result mean", "should we report this", "how much is at risk", and on any request to test a
  population against a business rule. Also use it when a finding needs retracting, when two
  sources disagree, or when someone asks why a number changed. Reach for it even when the request
  sounds like a simple one-off query — a single query that produces a number somebody will act on
  IS an audit, and the traps here are what make first-read numbers wrong. This is the half of the
  pipeline that comes after `audit-spec-builder`.
metadata:
  version: "0.1.0"
...
The catalogue of those failures is `references/failure-catalogue.md`. Read it before writing a
...
3. **Write the test with its own guard built in.** Every test carries at least one column that
   would expose it as broken — see `references/query-patterns.md`. The guard costs one column and
...
- **`references/failure-catalogue.md`** — the fourteen worked failures, each with the query that got
...
- **`references/query-patterns.md`** — six reusable SQL shapes with their guards.
- **`references/snowflake-columns.md`** — verified schema for the maids.cc warehouse, and the columns
