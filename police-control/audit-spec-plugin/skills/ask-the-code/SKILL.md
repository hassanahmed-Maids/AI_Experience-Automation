...
name: ask-the-code
description: >
  This skill should be used to query the maids.cc ERP codebase through the Low-Code Platform
  "Ask the Code" API, in order to discover native database table names, column names, join
  keys, and how a piece of ERP business logic actually works. Triggers on "ask the code",
  "ask the low-code platform", "what are the native table names", "where does the ERP store
  X", "which table holds X", "ask the ERP codebase", "check the ERP implementation", or
  whenever a spec needs a real ERP table or column name that is not in Snowflake. Also use
  when verifying that a business rule described by a user matches what the ERP code actually
  does. Requires a bearer token from the user, which expires within hours.
metadata:
  version: "0.1.0"
  verified: "Submit + poll verified working against erpbackendpro.maids.cc"
---
...
Read `references/module-registry.md` to pick modules, and `references/api-reference.md` for
