...
**Read `references/audit-spec-builder--spec-traps.md` before writing any verdict logic.** It carries eighteen defects
...
`references/audit-execution.md` skill in this plugin — and switching to it matters, because the failure modes are
different in kind.
...
- **Verdicts are six values, not two.** GREEN · RED · CANDIDATE · VOID · BLOCKED · REPORTED. A spec
  ...
  dashboard built from this spec will overstate coverage. See `references/audit-execution.md`.
...
- **A clearance is not a finding's opposite.** A record is clean only when every applicable test
  *ran* and passed; if one could not run, the record is amber. One red test outweighs any number of
  ...
  `references/audit-spec-builder--spec-traps.md` §1.
...
- **Ask the Code token** — required only if ERP table discovery becomes necessary. Ask the
  user for a fresh bearer token when Step 2 shows gaps. See the `references/ask-the-code.md` skill; tokens
...
Invoke the **`references/snowflake-discovery.md`** skill. Classify **every single data point** the report
...
**`references/ask-the-code.md`** skill and ask the low-code platform the pipeline's standard question:
...
This is a loop, not a single pass. Read `references/audit-spec-builder--feedback-loop.md` for the question banks.
...
Invoke the **`references/audit-report-ui.md`** skill. Produce a published artifact of the finished report
...
Write the spec to a markdown file using `references/audit-spec-builder--spec-template.md`. Every spec contains,
in this order:
...
it. Read `references/audit-spec-builder--dna-handoff.md` and produce:
