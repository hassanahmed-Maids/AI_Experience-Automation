  ...
  config: {
    ...
    parameters: {
      ...
      jsCode: '// The 15 LIVE deterministic gates, in ACP Order sequence. Order is the COLUMN, not the'
        ...
        + '\n' + '// A NODE ON AN UNTAKEN BRANCH IS UNEXECUTED, AND $() ON IT THROWS.'
        + '\n' + '// Get Extra Sweep Pages runs only when some maid has more than 20 complaints; Get Comment'
        + '\n' + '// Threads only when some complaint carries a comment. Neither is true on a quiet cohort -'
        + '\n' + '// which is the COMMON case - so reading them directly crashes the scorer on exactly the runs'
        + '\n' + '// most likely to happen. Pinned test data hides this completely, because a pinned node is'
        + '\n' + '// readable whether or not its branch was taken.'
        + '\n' + '//'
        + '\n' + '// Absent is EMPTY here, never an error: the No-Extra and No-Threads passthroughs are what'
        + '\n' + '// prove the branch was deliberately skipped. A read that FAILED, rather than being skipped,'
        + '\n' + '// still shows up as an unreconciled sweep or an unreadable thread further down, which is'
        + '\n' + '// where it belongs - it never silently becomes a clean.'
        + '\n' + 'function branchItems(name) { try { return $(name).all(); } catch (e) { return []; } }'
        + '\n' + 'const sweepExtra = branchItems("Get Extra Sweep Pages");'
        + '\n' + 'const threadPlan = $("Build Thread Requests").all();'
        + '\n' + 'const threadResp = branchItems("Get Comment Threads");'
        + '\n' + ''
  ...
  config: {
    ...
    parameters: {
      ...
      jsCode: '// THE RUN SUMMARY CARRIES COUNTS, FLAGS AND TOTALS ONLY. No amount, no maid id, no name.'
        ...
        + '\n' + '// ACTUAL ERP spend, counted from what each request node really emitted. The budget gate'
        + '\n' + '// projects a WORST CASE before any fan-out; without the actual beside it nobody can tell'
        + '\n' + '// whether the budget is about right or wildly conservative, so it never gets tuned.'
        + '\n' + '// Branch nodes are counted safely: unexecuted means zero calls, not a crash.'
        + '\n' + 'function calls(name) { try { return $(name).all().length; } catch (e) { return 0; } }'
        + '\n' + 'const erpCalls = calls("Get Population Count") + calls("Get Population Pages")'
        + '\n' + '  + calls("Get Switcher Count") + calls("Get Switcher Pages")'
        + '\n' + '  + calls("Get Maid Profile") + calls("Get Salary Rule") + calls("Get Payroll History")'
        + '\n' + '  + calls("Get Renew Documents") + calls("Get Complaints Page 0")'
        + '\n' + '  + calls("Get Extra Sweep Pages") + calls("Get Comment Threads");'
        ...
        + '\n' + '  erp_calls: erpCalls, erp_budget: params.erp_call_budget,'
