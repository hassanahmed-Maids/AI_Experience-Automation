...
**Three tickets to file** (pre-split, blocks links set), **three held** with a named unblock.
...
# ① Analytic Engineer Task

> **Title:** `Visa expense audit — consolidated silver model: shared layer plus four Police & Control audits`
...
> **Routing:** BI manager. **Blocked by:** ① (AE).
...
> **The ask: one Streamlit app over the silver model from ①. Eight tabs — a portfolio, then one per
...
# Held back — drafted, not filed

| Audit | What unblocks it | Then file |
|---|---|---|
| **LAWP v4** | Re-run `spec-auditor` on v4 (it has only ever run on v1, and v4 changed the finding definition, the tracked unit, the metric set and added R18/R19/R20). **Plus** open item 3: reconcile against `LOST_VISA_EXPENSES` — the spec itself says *"reconcile before shipping"*. **Plus** the R20a ruling from Mohammad Khalil, which decides **AED 383,177.57** | An AE ticket in the same shape, adding `V_CASES_LAWP` and its four new metrics |
| **E-ID v2** | Re-run `spec-auditor` — its own open item 7. The twice-run clearance covers v1; v2 adds audit 4. **Plus** the Visa team's answer to *what does a one-year E-ID cost*, which decides whether the 58 M8 cases are a finding at all | An AE ticket adding `V_CASES_EID` and M8 |
| **R-visa v7** | Three measurements (`Q-TERM`, `Q-FINE`, `Q-ORPHAN`), one Ask-the-Code answer (§9.7), five decisions (§6.a). Its own §0.7: *"Two of the three flowchart legs read SPECIFIED, NOT MEASURED — not BUILT. The corrected DNA pair cannot be raised on this."* | The corrected pair Hassan Okasha said would follow `DNA-9529` |
