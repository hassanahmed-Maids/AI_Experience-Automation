...
function scoreCase(input, options) {
  ...
  for (let i = 0; i < records.length; i++) {
    if (records[i].verdict === null) {
      ...
      records[i].gate = '⓭';
      records[i].reason = records[i].annotations.length
        ? 'unsettled:' + records[i].annotations[0]
        : 'unsettled-no-gate-matched';
    }
  }
  ...
  const allVerdicts = records.map(function (r) { return r.verdict; })
    ...
    .concat(verifier.map(function (v) { return v.verdict; }));
...
module.exports = {
  ...
  resolveBaseFee: resolveBaseFee,
  resolveAnchor: resolveAnchor,
  isRenewHead: isRenewHead,
  isRvisaExpense: isRvisaExpense,
  ...
  CONSTANTS: {
    ...
    RENEWAL_GAP_DAYS: RENEWAL_GAP_DAYS,
    DUPLICATE_GAP_DAYS: DUPLICATE_GAP_DAYS,
    TAXONOMY_CUTOVER: TAXONOMY_CUTOVER
  }
};
