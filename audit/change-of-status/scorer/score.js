...
function scoreRow(row) {
  ...
  if (!out.date) {
    out.verdict = 'pending'; out.reason = 'unreadable transaction date'; out.gate = 'Order 128';
    return out;
  }
  ...
  if (!pure.pure) {
    out.verdict = 'pending'; out.reason = pure.reason; out.gate = 'Order 25';
    return out;
  }
  ...
  if (typeof row.amount === 'number' && row.amount < 0) {
    out.verdict = 'pending'; out.reason = 'negative amount (reversal)'; out.gate = 'Order 128';
  ...
  if (base === null) {
    out.verdict = 'pending'; out.reason = 'off-era: no base band matches this date'; out.gate = 'Order 128';
  ...
  if (typeof row.amount !== 'number' || !isFinite(row.amount)) {
    out.verdict = 'pending'; out.reason = 'amount missing or unreadable'; out.gate = 'Order 20';
...
function applyDuplicateRule(scored, history) {
  ...
  for (const row of scored) {
    ...
    if (gap <= DUPLICATE_WINDOW_DAYS) {
      ...
      if (row.verdict === 'clean') { row.verdict = 'pending'; row.reason = band; row.gate = 'Order 125'; }
